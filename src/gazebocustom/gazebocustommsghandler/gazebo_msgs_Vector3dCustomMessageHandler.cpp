...
gazebo_msgs_Vector3dCustomMessageHandler::gazebo_msgs_Vector3dCustomMessageHandler(gazebo::transport::NodePtr node)
    : m_node(node)
    , m_mutex()
{
}
...
/// Destructor
gazebo_msgs_Vector3dCustomMessageHandler::~gazebo_msgs_Vector3dCustomMessageHandler() {
}

void gazebo_msgs_Vector3dCustomMessageHandler::setCustomPub(gazebo::transport::PublisherPtr customPubPtr,
                                          std::string const& topicName) {
    std::lock_guard<std::mutex> lock(m_mutex);
    m_customPublisherMap.insert(std::make_pair(topicName, customPubPtr));
}

gazebo::transport::PublisherPtr gazebo_msgs_Vector3dCustomMessageHandler::getCustomPub(std::string const& topicName) {
    {
        std::lock_guard<std::mutex> lock(m_mutex);
...
std::string gazebo_msgs_Vector3dCustomMessageHandler::initPublisher(mw::internal::robotics::gazebotransport::Packet const& msgContent) {
    ...
    if (iter != m_customPublisherMap.end()) {
        ...
        this->setCustomPub(this->m_node->Advertise<gazebo::msgs::Vector3d>(topicName),
                           topicName);
        replyMsg.set_status(mw::internal::robotics::gazebotransport::Packet_CoSimError_NONE);
    }
...
std::pair<std::shared_ptr<gazebo_msgs_Vector3dSubscriber>, bool> gazebo_msgs_Vector3dCustomMessageHandler::insert(
...
std::shared_ptr<gazebo_msgs_Vector3dSubscriber> gazebo_msgs_Vector3dCustomMessageHandler::find(std::string const& topic) {
    std::lock_guard<std::mutex> lock(m_mutex);
...
std::string gazebo_msgs_Vector3dCustomMessageHandler::initSubscriber(mw::internal::robotics::gazebotransport::Packet const& msgContent) {
    ...
    if (iter != m_subscriberPtrMap.end()) {
        ...
        auto insertResult =
            this->insert(topicName, std::make_shared<gazebo_msgs_Vector3dSubscriber>(topicName));
...
std::string gazebo_msgs_Vector3dCustomMessageHandler::publishCustomMsg(mw::internal::robotics::gazebotransport::Packet const& msgContent) {
    ...
    if (pubPointer) {
        gazebo::msgs::Vector3d gazebo_msgs_Vector3d_message;
        if (gazebo_msgs_Vector3d_message.ParseFromString(dataString)) {
            pubPointer->Publish(gazebo_msgs_Vector3d_message);
            // assign no error
            replyMsg.set_status(mw::internal::robotics::gazebotransport::Packet_CoSimError_NONE);
        } else {
...
std::pair<bool,std::string>  gazebo_msgs_Vector3dCustomMessageHandler::subscribeCustomMsg(mw::internal::robotics::gazebotransport::Packet const& msgContent) {
